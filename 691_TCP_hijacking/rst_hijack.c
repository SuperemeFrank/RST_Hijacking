...
int main(int argc, char *argv[]) { 
	...
	set_packet_filter(pcap_handle, (struct in_addr *)&target_ip);  // set filter
...
void caught_packet(u_char *user_args, const struct pcap_pkthdr *cap_header, const u_char *packet) {
    ...
    libnet_build_ip(LIBNET_TCP_H, 				// Size of the packet sans IP header 
		...
		*((u_long *)&(IPhdr->ip_dst)), 			// Source IP (pretend we are dst)			//here to pretend 
		*((u_long *)&(IPhdr->ip_src)), 			// Destination IP (send back to src)
		NULL,								  	// Payload (none)
		0,										// Payload length
		passed->packet);						// Packet header memory
    printf("Reconstruct Packet : \n");
    printf("src : %s  ", inet_ntoa(IPhdr->ip_src), inet_ntoa(IPhdr->ip_dst));
